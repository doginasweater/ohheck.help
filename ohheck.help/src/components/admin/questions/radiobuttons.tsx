...
export default class NewRadioButtons extends React.Component<any, any> {
    ...
    renderAnswers = (): JSX.Element[] => this.props.question.answers.sort(this.answerSort).map((item: Answer, index: number) =>
        <div className="pure-u-1" key={index}>
            <div className="pure-u-2-24">
                <label htmlFor={`radio-item-${this.props.index}-${index}`}>{index + 1}</label>
            </div>
            <div className="pure-u-14-24">
                <input type="text"
                    id={`radio-item-${this.props.index}-${index}`}
                    name={`${item.id}`}
    ...
    render() {
        return (
            <fieldset>
                <legend>Radio Buttons - Allows for a single choice from a set of buttons</legend>
